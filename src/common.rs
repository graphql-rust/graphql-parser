...
#[derive(Debug, Clone, PartialEq)]
pub struct Value<'a, T: Text<'a>> {
    pub position: Pos,
    pub kind: ValueKind<'a, T>,
}

impl<'a, T: Text<'a>> Value<'a, T> {
    pub fn new(position: Pos, kind: ValueKind<'a, T>) -> Self {
        Value { position, kind }
    }
}

#[derive(Debug, Clone, PartialEq)]
pub enum ValueKind<'a, T: Text<'a>> {
    Variable(T::Value),
    Int(Number),
    Float(f64),
    String(String),
...
impl<'a, T: Text<'a>> Value<'a, T> {
    pub fn into_static(&self) -> Value<'static, String> {
        Value::new(
            self.position,
            match &self.kind {
                ValueKind::Variable(v) => ValueKind::Variable(v.as_ref().into()),
                ...
                ValueKind::Enum(v) => ValueKind::Enum(v.as_ref().into()),
                ValueKind::List(l) => ValueKind::List(l.iter().map(|e| e.into_static()).collect()),
                ValueKind::Object(o) => ValueKind::Object(
                    o.iter()
    ...
    S: Text<'a>,
{
    (
        ...
        kind(T::IntValue)
            ...
            .map(|value| ValueKind::Int(Number(value))),
    )
        .map(|(pos, value)| Value::new(pos, value))
        .parse_stream(input)
}
    ...
    S: Text<'a>,
{
    (
        ...
        .map(|(position, value)| Value::new(position, value))
        .parse_stream(input)
}
    ...
    T: Text<'a>,
{
    (
        position(),
        ident("true")
            .map(|_| ValueKind::Boolean(true))
            .or(ident("false").map(|_| ValueKind::Boolean(false)))
            .or(ident("null").map(|_| ValueKind::Null))
            .or(name::<'a, T>().map(|n| ValueKind::Enum(n))),
    )
        .map(|(position, value)| Value::new(position, value))
        .or(parser(int_value))
